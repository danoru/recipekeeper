...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "POST") {
      await prisma.likedRecipes.upsert({
        where: { userId_recipeId: { userId: user.id, recipeId } },
        update: {},
        create: { userId: user.id, recipeId },
      });
    } else {
      await prisma.likedRecipes.deleteMany({ where: { userId: user.id, recipeId } });
    }
    await revalidateUserPages(res, user.username, ["/likes"]);
    return res.status(200).json({ message: "Likes updated." });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Failed to update liked recipes." });
  }
}
